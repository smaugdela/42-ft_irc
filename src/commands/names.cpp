...
static bool	visible_member(Channel *channel, Client *sender)
{
	...
	{
		if (it->second->getMode().find('i') == std::string::npos || it->second == sender)
			return true;
	}
	return false;
}
...
void names(Server *serv, Message &msg)
{
	...
	else
	{
		...
		{
			if (visible_member(serv->getChannel(*it), msg.getSender()) == true)
			{
				...
				for (; it2 != members.end(); it2++)
				{
					if (it2->second->getMode().find('i') == std::string::npos || it2->second == msg.getSender() || serv->getChannel(*it)->getMember(msg.getSender()->getNickname()))
					{
